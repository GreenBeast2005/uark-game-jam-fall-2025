...
@onready var resource: DialogueResource


func _ready():
	var resource_path = DialogueSettings.get_user_value("run_resource_path")
	if resource_path == null or resource_path == "":
		print("Error: No valid resource path found")
		return
	
	resource = load(resource_path)
	if resource == null:
		print("Error: Failed to load resource at path: " + str(resource_path))
		return
